...
async def process_single_chunk(
    chunk_idx, start_page, end_page, pdf_bytes, refId, fileName, start_chunk_page
):
    try:
        ...
        if chunk_idx == 0 and start_page is not None and end_page is not None:
            ...
            schema, prompt, master_schema = schema_prompt(
        ...
        if chunk_idx == 3:
            ...
            data1 = clean(
                master_schema=master_schema, data1=data1, data2=data2, data3=data3
            )
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                f"{CALLBACK_URL}/g{section}", json=payload, timeout=60.0
            )
...
async def background_manager(body: ChunkRequest, pdf_bytes: bytes):
    ...
    start_chunk_page = [
        v
        for k, v in locate_chunks(pdf_bytes=pdf_bytes, debug=False).items()
        if k != "last_page"
    ]
    tasks = []
